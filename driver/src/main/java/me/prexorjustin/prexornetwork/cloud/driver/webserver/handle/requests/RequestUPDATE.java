        ...
        if (uri.contains("/")) {
            ...
            if (authenticatorKey.length() > 4 && Driver.getInstance().getWebServer().AUTH_KEY.equals(authenticatorKey)) {
                ...
                if (Driver.getInstance().getWebServer().getRoutes(path) == null) {
                    ...
                    Driver.getInstance().getMessageStorage().getEventDriver().executeEvent(new CloudRestAPIUpdateEvent(path, payload));
                    NettyDriver.getInstance().getNettyServer().sendToAllSynchronized(new PacketOutCloudRestAPIUpdateEvent(path, payload));
