...
public class HandleVelocityPacketOutAPIPlayerTab implements NettyAdaptor {

    @Override
    public void handle(Channel channel, Packet packet) {
        if (packet instanceof PacketOutAPIPlayerTab packetCast) {
            Player player = VelocityBootstrap.getInstance().getProxyServer().getPlayer(packetCast.getUsername()).orElse(null);
            assert player != null;

            if (player.isActive())
                player.sendPlayerListHeaderAndFooter(
                        Component.text(packetCast.getHeader()),
                        Component.text(packetCast.getFooter())
                );
        }
    }
}
